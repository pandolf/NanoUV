...
int main( int argc, char* argv[] ) {
  ...
  std::vector< GunScan* > scans_nominal  = GunScan::loadScans( Form("data/%s/scans.txt", data.c_str()), gunEnergy, APDhv );
  std::vector< GunScan* > scans_syst  = GunScan::loadScans( Form("data/%s/scans.txt", data.c_str()), gunEnergy, APDhv, "pol2" );

  TGraphErrors* gr_iapd_vs_igun_nominal  = new TGraphErrors(0);
  gr_iapd_vs_igun_nominal->SetName("iapd_vs_igun_nominal");
  TGraphErrors* gr_iapd_vs_igun_syst  = new TGraphErrors(0);
  gr_iapd_vs_igun_syst->SetName("iapd_vs_igun_syst");


  for( unsigned iScan=0; iScan<scans_nominal.size(); iScan++ ) {

    if( scans_nominal[iScan]->gunEnergy() != gunEnergy ) continue;
    if( scans_nominal[iScan]->APDhv() != APDhv ) continue;

    scans_nominal[iScan]->set_currentMethod(currentMethod);
    scans_syst[iScan]->set_currentMethod(currentMethod);

    scans_nominal[iScan]->addPointToGraph( gr_iapd_vs_igun_nominal );
    scans_syst[iScan]->addPointToGraph( gr_iapd_vs_igun_syst );

  }


  TGraphErrors* gr_syst = new TGraphErrors(0);
  gr_syst->SetName("syst");


  for( unsigned i=0; i<gr_iapd_vs_igun_nominal->GetN(); i++ ) {
    double x, y_nominal, y_syst;
    gr_iapd_vs_igun_nominal->GetPoint( i, x, y_nominal );
    gr_iapd_vs_igun_syst->GetPoint( i, x, y_syst );
    gr_syst->SetPoint(i, x, abs(y_nominal-y_syst)/y_nominal );
  }

  TFile* outfile = TFile::Open(Form("systFuncForm_E%.0feV_APDhv%.0fV.root", gunEnergy, APDhv), "recreate");
  outfile->cd();
  gr_syst->Write();
  outfile->Close();
...
//
//float G_gamma = 44.0;
//float err_G_gamma = 0.1;
//float epsilon = 3.66;
//float E_d = epsilon*f1_line->GetParameter(1)/G_gamma;
//float err_E_d = sqrt( epsilon*epsilon*f1_line->GetParError(1)*f1_line->GetParError(1)/(G_gamma*G_gamma) + epsilon*epsilon*f1_line->GetParameter(1)*f1_line->GetParameter(1)*err_G_gamma*err_G_gamma/(G_gamma*G_gamma*G_gamma*G_gamma));

//std::cout << std::endl;
//std::cout << "-----------------------------------------------" << std::endl;
//std::cout << "E(d) = " << E_d << " +/- " << err_E_d << " eV" << std::endl;
//std::cout << "-> DeltaE = " << gunEnergy - E_d << " +/- " << err_E_d << " eV" << std::endl;
//std::cout << "-----------------------------------------------" << std::endl;
